 ...
 ******************************************************************************
  ...
  const static char cvsid[] = "$Id: spColMat.h,v 1.4 2005/08/20 00:52:39 xhu Exp $";
...
void vecMultMat_fv (T2 ans[],
                    const T1 fullVec[],
                    const SpColMat<T2> A,
                    const T2 zeroVal)
{
  for (size_t col=0; col<A.numCol(); col++)
  {
    ans[col] = zeroVal;
    for (size_t row = 0; row < A[col].size(); row++)
      ans[col] = ans[col]+A[col].value(row) * fullVec[ A[col].index(row)];
  }
}

template <class T1, class T2>
void matTMultVec_fv(T2 ans[],
                    const SpColMat<T1> A,
					const std::vector<T2> fullVec)
{
	for (size_t col=0; col<A.numCol(); col++)
	{
		ans[col]=0.;
...
template<class T>
		void output_to_file_complex (char* filename, const SpColMat<T>& a)
{
	...
	{
		for(size_t row=0; row<a[col].size(); row++)
			fprintf(fp, "%i %i %e %e \n ", a[col].index(row)+1, col+1, real(a[col].value(row)),
					imag(a[col].value(row)));
	}
	fclose(fp);
}
