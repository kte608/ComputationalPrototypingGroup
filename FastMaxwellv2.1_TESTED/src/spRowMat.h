 ...
 ***************************************************************************/
#ifndef SP_ROW_MAT_H_
#define SP_ROW_MAT_H_
...
**********************************************************************/
template <class T>
SpRowMat<T>::SpRowMat (size_t numRow,
                       size_t numCol): numCol_(numCol), numNonZero_(0)
{
  mat.resize(numRow);
  if (! numCol_)
    numCol_ = numRow;
...
template<class T>
T* sp_ptr_to(const SpRowMat<T>& mat,
             const int rowIndex,
             const int colIndex)
{
  return &mat[rowIndex].value[colIndex];
}
