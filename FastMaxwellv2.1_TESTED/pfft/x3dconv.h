...
namespace pfft{
  ...
  /**********************************************************************
   * circulantIndexHankel --
   **********************************************************************/
//  inline int circulantIndexHankel(int index, int maxSize)
//  {
//    if(index == maxSize/2 ){
//      return 0;
//    } else {
//      return ( (index < maxSize/2) ? (index+maxSize/2-1) : (index-maxSize/2-1) );
//    }
//  }
  ...
  inline int circulantIndexHankel(int index, int maxSize)
  {
    if(index == maxSize/2 ){
      ...
      return ( (index < maxSize/2) ? (-index+maxSize/2-1) : (-index+maxSize/2+1+maxSize) );
    }
  }
  ...
  inline fftw_complex operator* (const fftw_complex& x, const fftw_complex& y)
  {
    fftw_complex a;
...
/**********************************************************************
 ...
 template<class Vector, class CalcKernel>
  void setupKerneldiag (const size_t *dim2,
			    const double *pace,
			    Vector& kernelxx, Vector& kernelzz,
			    const CalcKernel& f)
  {
    const size_t nx2 = dim2[0];
    const size_t ny2 = dim2[1];
    ...
    //    const int length1D2 = nx2 * ny2 * nz2;
// add the coefficients and exponentials to the calculated function
    for(size_t ii=0; ii<nx2; ++ii){
      for(size_t jj=0; jj<ny2; ++jj){
	for(size_t kk=0; kk<nz2; ++kk){
	  int index2 = kk + nz2*(jj + ny2*ii);
	  TCopy( kernelxx[index2],
		 f.gxx(circulantIndex(ii, nx2) * dx,
		   circulantIndex(jj, ny2) * dy,
		 //  circulantIndex(kk, nz2) * dz) );
		   circulantIndexHankel(kk, nz2) * dz) );
	  TCopy( kernelzz[index2],
		 f.gzz(circulantIndex(ii, nx2) * dx,
		   circulantIndex(jj, ny2) * dy,
		 //  circulantIndex(kk, nz2) * dz) );
		   circulantIndexHankel(kk, nz2) * dz) );
	}
 ...
 template<class Vector, class CalcKernel>
  void setupKerneloffdiag (const size_t *dim2,
			    const double *pace,
			    Vector& kernelxz, Vector& kernelyz,
			    const CalcKernel& f)
  {
    const size_t nx2 = dim2[0];
    const size_t ny2 = dim2[1];
...
// add the coefficients and exponentials to the calculated function
	...
	for(size_t kk=0; kk<nz2; ++kk){
	  int index2 = kk + nz2*(jj + ny2*ii);
	  TCopy( kernelxz[index2],
		 f.gxz(circulantIndex(ii, nx2) * dx,
		   circulantIndex(jj, ny2) * dy,
		 //  circulantIndex(kk, nz2) * dz) );
		   circulantIndexHankel(kk, nz2) * dz) );
	  TCopy( kernelyz[index2],
		 f.gyz(circulantIndex(ii, nx2) * dx,
		   circulantIndex(jj, ny2) * dy,
		 //  circulantIndex(kk, nz2) * dz) );
		   circulantIndexHankel(kk, nz2) * dz) );
	}
  ...
  template<class Vector, class CalcKernel>
  void setupKernelHankel (const size_t *dim2,
			    const double *pace,
			    Vector& kernel,
			    const CalcKernel& f)
  {
...
// add the coefficients and exponentials to the calculated function
	...
	for(size_t kk=0; kk<nz2; ++kk){
	  int index2 = kk + nz2*(jj + ny2*ii);
	  TCopy( kernel[index2],
		 f.images(circulantIndex(ii, nx2) * dx,
		   circulantIndex(jj, ny2) * dy,
		 //  circulantIndex(kk, nz2) * dz) );
		   circulantIndexHankel(kk, nz2) * dz) );
	}
