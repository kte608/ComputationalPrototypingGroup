...
GridElement::mapElementToGridPoint(
				   const Grid& grid)
{
  //tarek moselhy to remove precorrection
  //maxthreshold=grid.gridStep();
				     ...
				     GridElement& guest)
{
  ...
  // maxthreshold = sqrt(3.)*(grid.separationFactor()+2)*grid.gridStep();
  maxthreshold = sqrt(2.)*(grid.separationFactor())*grid.gridStep();
  // maxthreshold = (grid.separationFactor())*grid.gridStep();
  double maxRadius = 0.;
  for (int hx = 0; hx < static_cast<int>(grid.numPointX()); hx++)
	...
	if (! gridElementMap[hostGridPoint].empty()) {
	  maxRadius = findMaxBoundingSphereRadius(hostGridPoint);
	  maxthreshold = max(maxthreshold,(grid.separationFactor()+2)*maxRadius);
	}
      }
	...
	if (! gridElementMap[hostGridPoint].empty()) {
	  ...
	  for (vector<int>::const_iterator 
		...
		if(AreClose((*elementListPtr)[*host], 
		  ...
		  //  double threshold = (grid.separationFactor() + 1) * (*elementListPtr)[*host].boundingSphereRadius() +
		  //    (*guest.elementListPtr)[*potentialNeighbour].boundingSphereRadius();
