		      ...
		      void)
{
  ...
  //double **tmp;
  //tmp = new double*[size];
  itpp::mat A(totalNumPoint_,numBasis_), AT(numBasis_,totalNumPoint_), AT_A(numBasis_,numBasis_),
    PinvA(numBasis_,totalNumPoint_);
  //for (int i=0; i<size; i++) tmp[i] = new double[size];
  for (int pointIndex = 0; pointIndex < totalNumPoint_; pointIndex++) 
    for (int polyTermIndex = 0; polyTermIndex < numBasis_; polyTermIndex++) 
      A(pointIndex,polyTermIndex) = basisValue_[pointIndex][polyTermIndex];
     // tmp[pointIndex][polyTermIndex] = basisValue_[pointIndex][polyTermIndex];
  //pseudo_inverse(tmp, totalNumPoint_, numBasis_);
  AT = itpp::transpose(A);
  AT_A = AT * A;
  PinvA = itpp::inv(AT_A) * AT;
  weight_ = TNT::Matrix<double>(numBasis_, totalNumPoint_);
  // rescale
  for (int polyTermIndex = 0; polyTermIndex < numBasis_; polyTermIndex++) 
    for (int pointIndex = 0; pointIndex < totalNumPoint_; pointIndex++) 
      weight_[polyTermIndex][pointIndex] = 
	PinvA(polyTermIndex,pointIndex) * equaScale_[pointIndex];
...
{
  int size = max(numBasis_, totalNumPoint_);
  TNT::Matrix<double> A(totalNumPoint_,numBasis_), AT(numBasis_,totalNumPoint_), AT_A(numBasis_,numBasis_),
    PinvA(numBasis_,totalNumPoint_);
  for (int pointIndex = 0; pointIndex < totalNumPoint_; pointIndex++) 
    for (int polyTermIndex = 0; polyTermIndex < numBasis_; polyTermIndex++) {
      A[pointIndex][polyTermIndex] = basisValue_[pointIndex][polyTermIndex];
      AT[polyTermIndex][pointIndex] = A[pointIndex][polyTermIndex];
    }
  //AT = itpp::transpose(A);
  AT_A = AT * A;
  TNT::invert(AT_A);
					...
					const DifferentialOperator operatorType)
{
  ...
  if (ele.shape()==8) {
    if ((ele.getfil().l_dir().x() < 0.) || (ele.getfil().l_dir().y() < 0.) || (ele.getfil().l_dir().z() < 0.))
      sgndir = -1;
  switch (operatorType) {
  case D0_D0:
    for (int polyTermIndex = 0; polyTermIndex < numBasis_; polyTermIndex++) {
      sign=-1;
      for (size_t i=0; i < ele.shape(); i++) {
	if (i!=2 && i!=6) sign=sign*-1;
	//	sign=sign*-1;
	moments[polyTermIndex] +=
	  (sgndir * sign * intALL_basisFuncValue((ele.vertex(i) - origin) * polyScale_,polyTermIndex)
	   /	ele.getfil().filCrossA());
      } }
    break;
  case D_DX:
    for (int polyTermIndex = 0; polyTermIndex < numBasis_; polyTermIndex++) {
	...
	//	sign=sign*-1;
	moments[polyTermIndex] +=
	  (sgndir * sign * intYZ_basisFuncValue((ele.vertex(i) - origin) * polyScale_,polyTermIndex)
	   /	ele.getfil().filCrossA());
      } }
    break;
  case D_DY:
	...
	//sign=sign*-1;
	moments[polyTermIndex] +=
	  (sgndir * sign * intXZ_basisFuncValue((ele.vertex(i) - origin) * polyScale_,polyTermIndex)
	   /	ele.getfil().filCrossA());
      }
    }
    break;
  case D_DZ:
    errorMessage("Stencil::integrateBasisFuncOverElement()",
		 "D_DZ not implemented yet!");
    break;
  default:
  ...
  double nx=ele.getpanel().normal().x();
  double ny=ele.getpanel().normal().y();
  double nz=ele.getpanel().normal().z();
  if (nx!=0.)	{
    for (int polyTermIndex = 0; polyTermIndex < numBasis_; polyTermIndex++) {
      sign=-1;
      for (size_t i=0; i < ele.shape(); i++) {
	...
	sign=sign*-1;
	moments[polyTermIndex] +=
	  (sign * intAREAYZ_basisFuncValue((ele.vertex(i) - origin) * polyScale_,polyTermIndex)
	   /	ele.getpanel().area());
      }
    }
  } else if (ny!=0.)	{
	...
	sign=sign*-1;
	moments[polyTermIndex] +=
	  (sign * intAREAXZ_basisFuncValue((ele.vertex(i) - origin) * polyScale_,polyTermIndex)
	   /	ele.getpanel().area());
      }
    }
  } else if (nz!=0.)	{
	...
	sign=sign*-1;
	moments[polyTermIndex] +=
	  (sign * intAREAXY_basisFuncValue((ele.vertex(i) - origin) * polyScale_,polyTermIndex)
	   /	ele.getpanel().area());
      }
    }
  }
...
/**********************************************************************
 * intALL_basisFuncValue -- for filaments
 **********************************************************************/
inline double
Stencil::intALL_basisFuncValue (
			    const point3D& point,
			    const int polyTermIndex) const
{
    return
      (intpow(point.x(), xMonoOrder_[polyTermIndex]+1)/(xMonoOrder_[polyTermIndex]+1)
		*  intpow(point.y(), yMonoOrder_[polyTermIndex]+1)/(yMonoOrder_[polyTermIndex]+1)
		*  intpow(point.z(), zMonoOrder_[polyTermIndex]+1)/(zMonoOrder_[polyTermIndex]+1))
		/ (polyScale_* polyScale_* polyScale_);
}

/**********************************************************************
 * intAREAYZ_basisFuncValue --  for panels
 **********************************************************************/
inline double
Stencil::intAREAYZ_basisFuncValue (
			    const point3D& point,
			    const int polyTermIndex) const
{
    return
      (intpow(point.x(), xMonoOrder_[polyTermIndex])
		*  intpow(point.y(), yMonoOrder_[polyTermIndex]+1)/(yMonoOrder_[polyTermIndex]+1)
		*  intpow(point.z(), zMonoOrder_[polyTermIndex]+1)/(zMonoOrder_[polyTermIndex]+1))
		/ (polyScale_*polyScale_);
}
/**********************************************************************
 * intAREAXZ_basisFuncValue -- for panels
 **********************************************************************/
inline double
Stencil::intAREAXZ_basisFuncValue (
			    const point3D& point,
			    const int polyTermIndex) const
{
    return
...
/**********************************************************************
 * intAREAXY_basisFuncValue -- for panels
 **********************************************************************/
inline double
Stencil::intAREAXY_basisFuncValue (
			    const point3D& point,
			    const int polyTermIndex) const
{
    return
...
/**********************************************************************
 * intYZ_basisFuncValue -- for filaments, with derivative (d_dx) operator in Kernel
 **********************************************************************/
inline double
Stencil::intYZ_basisFuncValue (
			    const point3D& point,
			    const int polyTermIndex) const
{
  if (xMonoOrder_[polyTermIndex] == 0) {
...
/**********************************************************************
 * intXZ_basisFuncValue -- for filaments, with derivative (d_dy) operator in Kernel
 **********************************************************************/
inline double
Stencil::intXZ_basisFuncValue (
			    const point3D& point,
			    const int polyTermIndex) const
{
  if (yMonoOrder_[polyTermIndex] == 0) {
