			    ...
			    const size_t matIndex)
{
  ...
  TNT::Vector<double> integralOfBasisFunc(interpStencilPtr->numBasis());
  ...
  for (size_t ix=0; ix < gridPtr->numPointX(); ix++) {
	...
	for (size_t localElementIndex = 0; localElementIndex < numElement;
	  ...
	  interpStencilPtr->integrateBasisFuncOverElement(evalElement, origin,
							   integralOfBasisFunc,
							   operatorType);
	  interpStencilPtr->compCoef(integralOfBasisFunc, denseRow);
