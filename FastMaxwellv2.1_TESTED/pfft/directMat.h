...
namespace pfft {
  ...
  public:
    ...
    //SpColMat<complex<double> > getPreCondMat(void) {return PreCondMat;}
    std::vector<complex<double> > getPreCondMat(void) {return PreCondMat;}
  private:
    ...
    //pfft::SpColMat<complex<double> > PreCondMat;
    std::vector<complex<double> > PreCondMat;
    ...
    void precorrectRegularNeighborHankel (std::vector<size_t>& neighborIndex,
				      int numSrcElement,
				      int hostPoint1DIndex,
				      const GridIndex& hostPoint3DIndex,
				      GreenFunc greenFunc,
							...
							void)
  {
    ...
    // tarek moselhy beta SRC release: Q preconditioner before or after precorrection
    
    // PreCondMat=pfft::SpColMat<complex<double> >(matList[0].numCol(),matList[0].numRow());
    PreCondMat=std::vector<complex<double> >(matList[0].numRow());
    //if (srcGridElementPtr->getElement(0).shape()==8) {
    for (int i=0; i<matList[0].numCol(); i++)
      for (int j=0; j<matList[0][i].size(); j++) {
	if (i == matList[0][i].index(j)) {
	  PreCondMat[i]=matList[0][i].value(j);
	  //cout << PreCondMat[i] << endl;
	} 
      } 
    //}
							  ...
							  void)
  {
    ...
    //int numHIT=0; 
    //int numMISS=0;
    //std::map<vector<double>, int> directMatLookUp;
	...
	for (size_t i = 0; i < integrationPtr->numIntegral(); i++) {
	  ...
	  //vector<double> Z;
	  //Z.push_back(spElement.real());
	  //Z.push_back(spElement.imag());	  
	  //if (directMatLookUp.find(Z) != directMatLookUp.end())
	  //  numHIT++;
	  //else {
	  //  directMatLookUp.insert(std::pair<vector<double>, int>(Z, numMISS++));
	  // }
	      ...
	      G2GUnion<GreenFunc, KernelType>& g2gUnion)
  {
	...
	for (int z = 0; z < static_cast<int>(gridPtr->numPointZ()); z++) {
	  ...
	  precorrectIrregularNeighbor(neighborIndex, numSrcElement, hostPoint1DIndex,
				      hostPoint3DIndex, *greenFuncPtr, g2gMatList);
	  precorrectRegularNeighborHankel(neighborIndex, numSrcElement, hostPoint1DIndex,
				      hostPoint3DIndex, *greenFuncPtr, g2gMatList);
	//  precorrectRegularNeighbor(neighborIndex, numSrcElement, hostPoint1DIndex,
  ...
  void
  DirectMat<GreenFunc, Integration, KernelType>::
  precorrectRegularNeighborHankel (
			       std::vector<size_t>& neighborIndex,
			       int numSrcElement,
			       int hostPoint1DIndex,
			       const GridIndex& hostPoint3DIndex,
			       ...
			       std::map<GridIndex, TNT::Matrix<KernelType> >& g2gMatList)
  {
	...
	srcGridElementPtr->getElementMappedToOneGrid(hostPoint1DIndex,
      ...
      int numRegularNeighbor =
	srcGridElementPtr->numRegularNeighbor(srcElementGlobalIndex);
      if (numRegularNeighbor == 0) continue;

      for (int localNeighborIndex = 0; localNeighborIndex < numRegularNeighbor;
	   localNeighborIndex++) {
	int evalElementGlobalIndex =
	  srcGridElementPtr->getRegularNeighborGlobalIndex(srcElementGlobalIndex,
							     localNeighborIndex);
        int gridIndex =
	  evalGridElementPtr->gridMappedToElement(evalElementGlobalIndex);
	GridIndex guestPoint3DIndex = gridPtr->transfer1DIndexTo3D(gridIndex);
