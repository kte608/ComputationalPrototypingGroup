  ...
  directStencilPtr(directStencil), maxGridSize_(maxGridSize)
{
  ...
  double r1 = (domain_ymax_ - domain_ymin_) / (domain_xmax_ - domain_xmin_);
  double r2 = (domain_zmax_ - domain_zmin_) / (domain_xmax_ - domain_xmin_); 
  double numPoint = log(max(srcGridElementPtr->numElement(),evalGridElementPtr->numElement())/(r1 * r2))/ (3 * log(2.));

  //  numPointX_ = 64; 
  //  numPointY_ = 64;
  //  numPointZ_ = 4;
  //  bakNumPointX_ = numPointX_;
  //  bakNumPointY_ = numPointY_;
  //  bakNumPointZ_ = numPointZ_;


  numPointX_ = (size_t) max(4., pow(2., ceil(numPoint))); 
  numPointY_ = (int) max(4., pow(2.,ceil(log(r1 * pow(2,numPoint))/log(2.))));
  numPointZ_ = (int) max(4., pow(2.,ceil(log(r2 * pow(2,numPoint))/log(2.))));
