...
void parse_command_line(argv, argc, autmom, autlev, relperm, numMom, numLev, 
			input_file, surf_list_file, read_from_stdin, gsize,Rcoef)
...
double *Rcoef; 
{
  ...
  for(i = 1; i < argc && cmderr == FALSE; i++) {
	...
	*read_from_stdin = TRUE;
	...
	if(sscanf(&(argv[i][2]), "%lf", &gpz0) != 1) cmderr = TRUE;
        printf("interface plane at z = %g\n", gpz0); 
	if(cmderr) {
	  fprintf(stderr, "%s: bad interface plane position `%s'\n", argv[0], &argv[i][2]);
	  break;
	}
      }
      else if(argv[i][1] == 'R') {
	if(sscanf(&(argv[i][2]), "%lf", Rcoef) != 1) cmderr = TRUE;
        printf("interface plane R-coef = %g\n", *Rcoef); 
	if(cmderr) {
	  fprintf(stderr, "%s: bad interface R-coef `%s'\n", argv[0], &argv[i][2]);
	  break;
	}
      }
#endif 
...
surface *input_surfaces(argv, argc, autmom, autlev, relperm, 
			numMom, numLev, infile,gsize,Rcoef)
...
#if GNDPLN == ON 
  /* should have input charges & read in interface offset */ 
  { 
    charge *p; 
    printf("correcting panel z positions [%g]\n", gpz0); 
    for (p=chglist; p != NULL; p = p->next) { 
      p->z -= gpz0; 
      if (p->z < 0) { 
        fprintf(stderr, "Warning: a panel is in the wrong half-space; results may be invalid\n"); 
      }
    }
  }
#endif 
