...
/*
  fprintf(stdout, 
   "\nM2L MATRIX: multi at (%.15e %.15e %.15e) -> local at (%.15e %.15e %.15e)\n",
	  x, y, z, xp, yp, zp);
  dumpMat(mat, multerms(order), multerms(order));
*/

  matlabDumpjkw(mat, multerms(order), fp);
...
*/
  ...
  for(j = ind = 0; j < size; j++) {
    for(i = 0; i < size; i++) {
      temp[ind++] = mat[i][j];
    }
  }
  savemat(fp, 1000, "A", size, size, 0, temp, (double *)NULL);
...
FILE *fp;
{
  ...
  for(panel = chglist; panel != NULL; panel = panel->next) {
    ...
    fprintf(fp, "%d %20.15g %.3g %.3g %.3g\n", panel->index,  q[panel->index], panel->x, panel->y,panel->z); 
/*
...
/*
  ...
  for(i = 1; i <= numconds; i++) { /* rows */
	...
	fprintf(stdout, unit, scale*FPIEPS*relperm*sym_mat[j][i]);
        fprintf(fp, unit, scale*FPIEPS*relperm*sym_mat[j][i]);
      }
    }
    fprintf(stdout, "\n");
    fprintf(fp, "\n");
