...
#if (1==0) 
    if (pq->shape == 3) { 
      vtemp[XI] = pq->corner[0][XI] - pq->corner[2][XI]; 
      vtemp[YI] = pq->corner[0][YI] - pq->corner[2][YI]; 
      vtemp[ZI] = pq->corner[0][ZI] - pq->corner[2][ZI]; 
      normalize(vtemp); 
      printf("edge (0,2) [X] = %10g\n", vtemp[XI]); 
      printf("edge (0,2) [Y] = %10g\n", vtemp[YI]); 
      printf("edge (0,2) [Z] = %10g\n", vtemp[ZI]); 
    }
#endif 
...
double x, y, z, *pfd;
{
  ...
  struct edge *edge;
  double *corner;
  double vertex[6], eps, ans, err, rdata[6], ctmp; 
...
dumpnums2()
{
    fprintf(stdout, "Potential coefficient counts\n multipole only:\n");
    fprintf(stdout, 
	    "  2nd order: %d  4th: %d  Integral: %d \n",
	    num2nd, num4th,  numexact); 
}
