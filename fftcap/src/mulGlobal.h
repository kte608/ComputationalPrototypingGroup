...
{                                                                           \
	 ...
	 "\nfastcap: out of memory in file `%s' at line %d\n",              \
       ...
       else if(MTYP == ACUB) memCUB += ((NUM)*sizeof(TYPE));                \
       else if(MTYP == ACHG) memCHG += ((NUM)*sizeof(TYPE));                \
       else {                                                               \
...
/* FCALLOC --  only use for mem immediately to be free; not counted! */ 
#define FCALLOC(PNTR, NUM, TYPE, FLAG, MTYP)                                 \
{                                                                           \
     if((NUM)*sizeof(TYPE)==0)                                              \
       (void)fprintf(stderr,                                                \
		     "zero element request in file `%s' at line %d\n",      \
		     __FILE__, __LINE__);	                            \
     else if(((PNTR)=(TYPE*)calloc((size_t)NUM, sizeof(TYPE)))==NULL) {                    \
       (void)fprintf(stderr,                                                \
	 "\nfastcap: out of memory in file `%s' at line %d\n",              \
	       __FILE__, __LINE__);                                         \
       (void)fprintf(stderr, " (NULL pointer on %d byte request)\n",        \
...
#define NNBRS 1			/* Distance to consider a nearest nbr. */
#define ADAPT OFF		/* ON=> use adaptive algorithm */
...
#define RAWDAT ON		/* ON=> dump unsymm, Gaussian units cap mat */
...
extern double GroundPlaneHeight; 
extern double Rcoef; 
