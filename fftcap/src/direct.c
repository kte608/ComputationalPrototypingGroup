...
double **Q2PDiag(chgs, numchgs, calc)
charge **chgs;
int numchgs;
int calc;
{
  ...
  double calcp();
  double tmp, tmp1; 
  double vertex[6], eps, ans, *w, err, rdata[6]; 
  int mcalls, ncalls, nw, ier, k, idata[2]; 
  double xc,yc,zc, xn, yn, zn; 
  charge *panel; 
  /* Allocate storage for the potential coefficients. */
  CALLOC(mat, numchgs, double*, ON, AQ2P);
  for(i=0; i < numchgs; i++) CALLOC(mat[i], numchgs, double, ON, AQ2P);
...
#define DotP_Product(V1,R,S,T) ((V1[XI])*(R)+(V1[YI])*(S)+(V1[ZI])*(T))
	...
	mat[i][j] = calcp(chgs[j], chgs[i]->x, chgs[i]->y, 
			  chgs[i]->z, NULL);
        
        num_calcp_panel++; 
#if GNDPLN == ON 
	mat[i][j] += Rcoef * calcp(chgs[j], chgs[i]->x, chgs[i]->y, 
			  -1.0*chgs[i]->z, NULL);
#endif 
      }
    }
...
#if DSQ2PD == ON
  dispQ2PDiag(mat, chgs, numchgs);
#endif
  return(mat);
}

double **Q2P(qchgs, numqchgs, pchgs, numpchgs, calc)
charge **qchgs, **pchgs;
...
int calc;
{
  ...
  for(i=0; i < numpchgs; i++) {
	...
	mat[i][j] = calcp(qchgs[j], pchgs[i]->x, pchgs[i]->y, 
        ...
        num_calcp_panel++; 
#if GNDPLN  == ON 
	mat[i][j] += Rcoef * calcp(qchgs[j], pchgs[i]->x, pchgs[i]->y, 
			  -1.0*pchgs[i]->z, NULL);
#endif 
      }
    }
...
#if GNDPLN == ON 
	  mat[i][j] += Rcoef * calcp(pchgs[i-fromp],eval->x, eval->y, -1.0*eval->z, NULL);
#endif 
	}
      }
